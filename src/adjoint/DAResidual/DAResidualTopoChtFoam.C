...
#include "DAResidualTopoChtFoam.H"
...
defineTypeNameAndDebug(DAResidualTopoChtFoam, 0);
addToRunTimeSelectionTable(DAResidual, DAResidualTopoChtFoam, dictionary);
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

DAResidualTopoChtFoam::DAResidualTopoChtFoam(
    ...
    : DAResidual(modelType, mesh, daOption, daModel, daIndex),
      ...
      diffusivityFluid_(const_cast<volScalarField&>(
          mesh_.thisDb().lookupObject<volScalarField>("alphat"))),
      diffusivity_(const_cast<volScalarField&>(
          mesh_.thisDb().lookupObject<volScalarField>("diffusivity"))),
      ...
      eta_(const_cast<volScalarField&>(
          mesh_.thisDb().lookupObject<volScalarField>("eta"))),
          ...
          mesh_.thisDb().lookupObject<IOMRFZoneListDF>("MRFProperties")))
{
    ...
    alphaPorosityFluid_ = readScalar(transportProperties.lookup("alphaPorosityFluid"));
    alphaPorositySolid_ = readScalar(transportProperties.lookup("alphaPorositySolid"));
    diffusivitySolid_ = readScalar(transportProperties.lookup("diffusivitySolid"));
    pCoeff_ = readScalar(transportProperties.lookup("pCoeff"));
}

// * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * //

void DAResidualTopoChtFoam::clear()
{
    /*
    Description:
...
void DAResidualTopoChtFoam::calcResiduals(const dictionary& options)
{
    /*
    Description:
...
void DAResidualTopoChtFoam::updateIntermediateVariables()
{
    /* 
    Description:
    ...
    */
    ...
    forAll(alphaPorosity_, cellI)
    {
        alphaPorosity_[cellI] = alphaPorositySolid_ + (alphaPorosityFluid_ - alphaPorositySolid_) * eta_[cellI] * (1 + pCoeff_) / (eta_[cellI] + pCoeff_);
    }
    alphaPorosity_.correctBoundaryConditions();

    // no need to correct diffusivityFluid_ because it is corrected in DATurbulenceModel
    volScalarField diffusivityFluidEff("alphaEff", daTurb_.nu() / Pr_ + diffusivityFluid_);

    forAll(diffusivity_, cellI)
    {
        diffusivity_[cellI] = diffusivitySolid_ + (diffusivityFluidEff[cellI] - diffusivitySolid_) * eta_[cellI] * (1 + pCoeff_) / (eta_[cellI] + pCoeff_);
    }
    diffusivity_.correctBoundaryConditions();
}

void DAResidualTopoChtFoam::correctBoundaryConditions()
{
    /* 
    Description:
