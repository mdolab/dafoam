...
{
    Info<< "MULESDF: Solving for " << psi.name() << endl;
...
)
{
    ...
    {
        forAll(sumlPhip, idxI)
        {
            sumlPhip[idxI] = 0;
        }
        forAll(mSumlPhim, idxI)
        {
            mSumlPhim[idxI] = 0;
        }
...
void Foam::MULESDF::limitSum(SurfaceScalarFieldList& phiPsiCorrs)
{
    {
