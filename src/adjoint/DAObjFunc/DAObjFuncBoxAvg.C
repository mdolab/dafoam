...
defineTypeNameAndDebug(DAObjFuncBoxAvg, 0);
addToRunTimeSelectionTable(DAObjFunc, DAObjFuncBoxAvg, dictionary);
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

DAObjFuncBoxAvg::DAObjFuncBoxAvg(
    const fvMesh& mesh,
...
void DAObjFuncBoxAvg::calcObjFunc(
    ...
    scalar& objFuncValue)
{
    ...
    */
    ...
    else if (data_ == "UData")
    {
        ...
        //const Foam::IOobject meshIO("target", polyMeshPath, mesh_.time(), Foam::IOobject::MUST_READ);
        ...
        IOobject(
            ...
            IOobject::MUST_READ)));
        */
        //const objectRegistry& db_target = target.thisDb();

        stateName_ = "U";
        const volVectorField state = db.lookupObject<volVectorField>(stateName_);
        //const volVectorField stateRef = db_target.lookupObject<volVectorField>("uData");

        //stateRefName_ = data_; 

        volVectorField stateRef(
                    IOobject(
                        "uData",
                        Foam::name(0),
                        target,
        ...
        Info<< "Creating field U_target\n" << endl;
        volVectorField U_target
        (
            IOobject
            (
                "U_target",
            ...
            target,
            dimensionedVector("target", dimensionSet(0,1,-1,0,0,0,0), Foam::vector(0.0, 0.0, 0.0))  // This is used to "size" the field to mesh domain.
        );

        /*Info<< "Read in reference data uData\n" << endl;
        volVectorField uData
        (
            IOobject
            (
                "uData",
        ...
        */
        Info<< "Creating field objfunctarget\n" << endl;
        volScalarField objfunctarget
        (
            IOobject
            (
                "objfunctarget",
            ...
            target,
            dimensionedScalar("objfunctarget", dimensionSet(0,2,-2,0,0,0,0), 0.0)  // This is used to "size" the field to mesh domain.

        );

        Info<< "Creating field objfuncsource\n" << endl;
        volScalarField objfuncsource
        (
            IOobject
            (
                "objfuncsource",
            ...
            mesh_,
            dimensionedScalar("objfuncsource", dimensionSet(0,2,-2,0,0,0,0), 0.0)  // This is used to "size" the field to mesh domain.

        );
        // create the meshtomesh interpolator object
        Info << "Interpolating U from CFD to experimental grid: " << endl;
        ...
        //do the interpolation between U of the source mesh and target mesh
        ...
        forAll(objfunctarget, idxI)
        {
            objfunctarget[idxI] = (sqr(mag(scale_ * stateRef[idxI] - U_target[idxI])));
            objFuncValue+=objfunctarget[idxI];
        }
        Info << "Interpolating objfunctarget from experimental to CFD grid: " << endl;
        meshToMesh InterpolatorObj_1
        (
        target,
        mesh_,
        Foam::meshToMesh::interpolationMethod::imCellVolumeWeight,
        ...
        forAll(objfuncsource, idxI)
        {
            objFuncCellValues[idxI] = objfuncsource[idxI];
        }
