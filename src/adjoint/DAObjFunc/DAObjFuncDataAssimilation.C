...
#include "DAObjFuncDataAssimilation.H"
...
defineTypeNameAndDebug(DAObjFuncDataAssimilation, 0);
addToRunTimeSelectionTable(DAObjFunc, DAObjFuncDataAssimilation, dictionary);
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

DAObjFuncDataAssimilation::DAObjFuncDataAssimilation(
    const fvMesh& mesh,
...
void DAObjFuncDataAssimilation::calcObjFunc(
    ...
    scalar& objFuncValue)
{
    ...
    */
    ...
    if (data_ == "beta")
    {
        stateName_ = "betaDataAssimilation";
        const volScalarField betaDataAssimilation_ = db.lookupObject<volScalarField>(stateName_);
        
        forAll(objFuncCellSources, idxI)
        {
            const label& cellI = objFuncCellSources[idxI];
            objFuncCellValues[idxI] = scale_ * (sqr(betaDataAssimilation_[cellI]));
