...
defineTypeNameAndDebug(DAkOmegaSSTSR, 0);
addToRunTimeSelectionTable(DATurbulenceModel, DAkOmegaSSTSR, dictionary);
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

DAkOmegaSSTSR::DAkOmegaSSTSR(
      ...
      betaFI_(
        ...
        correction_
            (
                ...
                mesh,
                dimensionedScalar("correction", dimensionSet(0, 0, 0, 0, 0, 0, 0), 0.0)
            ),
        PbyE_
            (
                IOobject
                ...
                mesh,
                dimensionedScalar("PbyE", dimensionSet(0, 0, 0, 0, 0, 0, 0), 0.0)
            ),
        epsilon_
            (
                IOobject
        ...
        betaSR_
            (
                ...
                mesh,
                dimensionedScalar("betaSR", dimensionSet(0, 0, 0, 0, 0, 0, 0), 0.0)
            ),
        SI_
            (
                IOobject
                ...
                mesh,
                dimensionedScalar("SI", dimensionSet(0, 0, 0, 0, 0, 0, 0), 0.0)
            ),
        SL_
            (
                IOobject
...
volScalarField DAkOmegaSSTSR::chiSR() 
{
    ...
    PbyE_ = max(scalar(-1.0) * ((((scalar(2.0)/scalar(3.0))*I)*k_ - nut_*dev(twoSymm(fvc::grad(U)))) && S) / (epsilon_ + low_eps_epsilon), scalar(0.0));

    correction_ = tanh((lambda2_ * lambda5_) / (reOmega_ + 1e-10)) * (PbyE_ - scalar(0.244));


    return correction_;
}

volScalarField DAkOmegaSSTSR::sLambda5() 
{
    SL_ = 0.5 * tanh(C_lamba5_1 * (lambda5_ - C_lamba5_2)) + 0.5;
    return SL_;
}

volScalarField DAkOmegaSSTSR::sI() 
{
    volVectorField U = mesh_.thisDb().lookupObject<volVectorField>("U");
    volScalarField I = k_ / (magSqr(U) + magSqr_eps);
    SI_ = 0.5 * tanh(C_I_1 * (I - C_I_2)) + 0.5;
    return SI_;
}

volScalarField DAkOmegaSSTSR::betaSR() 
{
    betaSR_ = chiSR() * sLambda5() * sI() + 1;
    return betaSR_;
}
...
tmp<volScalarField> DAkOmegaSSTSR::F1(
...
tmp<volScalarField> DAkOmegaSSTSR::F2() const
{
...
tmp<volScalarField> DAkOmegaSSTSR::F3() const
{
...
tmp<volScalarField> DAkOmegaSSTSR::F23() const
{
...
tmp<volScalarField::Internal> DAkOmegaSSTSR::GbyNu(
...
tmp<volScalarField::Internal> DAkOmegaSSTSR::Pk(
    const volScalarField::Internal& G) const
{
    return min(G, (c1_ * betaStar_) * k_() * omega_());
}

tmp<volScalarField::Internal> DAkOmegaSSTSR::epsilonByk(
...
tmp<fvScalarMatrix> DAkOmegaSSTSR::kSource() const
{
...
tmp<fvScalarMatrix> DAkOmegaSSTSR::omegaSource() const
{
    const volScalarField& rho = rho_;
    return tmp<fvScalarMatrix>(
        new fvScalarMatrix(
...
tmp<fvScalarMatrix> DAkOmegaSSTSR::Qsas(
...
// Augmented functions
void DAkOmegaSSTSR::correctModelStates(wordList& modelStates) const
{
    /*
    Description:
...
void DAkOmegaSSTSR::correctBoundaryConditions()
{
    /*
    Description:
...
void DAkOmegaSSTSR::correctOmegaBoundaryConditions()
{
    /*
    Description:
...
void DAkOmegaSSTSR::saveOmegaNearWall()
{
    /*
    Description:
...
void DAkOmegaSSTSR::setOmegaNearWall()
{
    /*
    Description:
...
void DAkOmegaSSTSR::updateIntermediateVariables()
{
    /*
    Description:
...
void DAkOmegaSSTSR::correctStateResidualModelCon(List<List<word>>& stateCon) const
{
    /*
    Description:
...
void DAkOmegaSSTSR::addModelResidualCon(HashTable<List<List<word>>>& allCon) const
{
    /*
    Description:
...
void DAkOmegaSSTSR::correct(label printToScreen)
{
    /*
    Descroption:
...
void DAkOmegaSSTSR::calcResiduals(const dictionary& options)
{
    /*
    Descroption:
    ...
    */
    ...
    volScalarField::Internal G("kOmegaSSTSR:G", nut_ * GbyNu0);
...
void DAkOmegaSSTSR::getFvMatrixFields(
    const word varName,
    scalarField& diag,
    scalarField& upper,
    scalarField& lower)
...
void DAkOmegaSSTSR::getTurbProdOverDestruct(scalarList& PoD) const
{
    /*
    Description:
...
void DAkOmegaSSTSR::getTurbConvOverProd(scalarList& CoP) const
{
    /*
    Description:
