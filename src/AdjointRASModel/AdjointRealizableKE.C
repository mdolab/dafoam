...
defineTypeNameAndDebug(AdjointRealizableKE, 0);
addToRunTimeSelectionTable(AdjointRASModel, AdjointRealizableKE, dictionary);
// * * * * * * * * * * * * * * * * Constructors  * * * * * * * * * * * * * * //

AdjointRealizableKE::AdjointRealizableKE
(
    const fvMesh& mesh,
...
// KE member functions
tmp<fvScalarMatrix> AdjointRealizableKE::kSource() const
{
...
tmp<fvScalarMatrix> AdjointRealizableKE::epsilonSource() const
{
    return tmp<fvScalarMatrix>
    (
        new fvScalarMatrix
...
tmp<volScalarField> AdjointRealizableKE::rCmu
(
...
void AdjointRealizableKE::updateNut()
{
    tmp<volTensorField> tgradU = fvc::grad(U_);
    volScalarField S2(2*magSqr(dev(symm(tgradU()))));
    volScalarField magS(sqrt(S2));
    nut_ =  rCmu(tgradU, S2, magS)*sqr(k_)/epsilon_;
...
void AdjointRealizableKE::copyTurbStates(const word option)
{
...
void AdjointRealizableKE::correctTurbBoundaryConditions()
{
...
void AdjointRealizableKE::correctEpsilonBoundaryConditions()
{
...
void AdjointRealizableKE::calcTurbResiduals
(  
    const label isRef,
    const label isPC,
...
{    
    ...
    (
     ...
     ==
        C1*phase*rho*magS*epsilon_
      - fvm::Sp
        (
            C2_*phase*rho*epsilon_/(k_ + sqrt(nuLimited*epsilon_)),
            epsilon_
        )
      + epsilonSource()
...
void AdjointRealizableKE::saveEpsilonNearWall()
{
...
void AdjointRealizableKE::setEpsilonNearWall()
{
...
void AdjointRealizableKE::correctAdjStateResidualTurbCon
(
...
void AdjointRealizableKE::setAdjStateResidualTurbCon
(
...
void AdjointRealizableKE::clearTurbVars()
{
...
void AdjointRealizableKE::writeTurbStates()
{
    nut_.write();
    k_.write();
    epsilon_.write();
