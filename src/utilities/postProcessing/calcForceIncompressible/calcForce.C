...
#include "createTime.H"
    ...
    {
        rho = readScalar(args.optionLookup("rho")());
    }
    else
    {
        Info << "Density not set! Defaulting to 1.0." << endl;
    }
...
#include "createFields.H"
        ...
        forAll(patchNames, cI)
        {
            ...
            // normal force
            vectorField fN(Sfb[patchI] * p.boundaryField()[patchI] * rho);
            // tangential force
            vectorField fT(Sfb[patchI] & devRhoReffb[patchI] * rho);
            // sum them up
            forAll(patch, faceI)
            {
                // compute forces
                ...
                volumeForceField.boundaryFieldRef()[patchI][faceI] = forces / magSfb[patchI][faceI];
