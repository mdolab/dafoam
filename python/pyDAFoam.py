...
class PYDAFOAM(AeroSolver):
    ...
    def aCompleteInputParameterSet(self):
        ...
        preservepatches : list
            Patches name for preservePatches which is needed for cyclic boundary conditions in parallel

        singleprocessorfacesets : list
            Face set name for singleprocessorfacesets which is needed for cyclicAMI boundary conditions in parallel
        ...
        defOpts = {  # a special key to add/modify default dictionary without explicitly write them all
                    ...
                    'fvsolvers': [dict, {'"(p|p_rgh|G)"': {'solver': 'GAMG',
                                         ...
                                         '"(p|p_rgh|G)Final"': {'solver': 'GAMG',
                                                                ...
                                                                'mergeLevels': '1',
                                                                'maxIter': '500'},
                                         '"(U|T|e|h|nuTilda|k|omega|epsilon|ReThetat|gammaInt|v2|f)"':
                                             {'solver': 'smoothSolver',
                                              'tolerance': '1e-20',
                                              'relTol': '0.1',
                                              'smoother': 'GaussSeidel',
                                              'nSweeps': '1',
                                              'maxIter': '10'},
                                         '"(U|T|e|h|nuTilda|k|omega|epsilon|ReThetat|gammaInt|v2|f)Final"':
                                             {'solver': 'smoothSolver',
                                              'tolerance': '1e-06',
                                              'relTol': '0',
                                              'smoother': 'GaussSeidel',
                    ...
                    'normalizeresiduals': [list, ['URes', 'pRes', 'p_rghRes', 'phiRes', 'TRes', 'nuTildaRes', 'kRes',
                                                  'omegaRes', 'epsilonRes', 'ReThetatRes', 'gammaIntRes', 'GRes',
                                                  'DRes', 'v2Res', 'fRes']],
                    'normalizestates': [list, ['U', 'p', 'p_rgh', 'phi', 'T', 'nuTilda', 'k', 'omega', 'epsilon',
                                               'ReThetat', 'gammaInt', 'G', 'D', 'v2', 'f']],
                    'maxresconlv4jacpcmat': [dict, {'URes': 2, 'pRes': 2, 'p_rghRes': 2, 'phiRes': 1, 'nuTildaRes': 2,
                                                    'kRes': 2, 'omegaRes': 2, 'epsilonRes': 2, 'ReThetatRes': 2,
                                                    'gammaIntRes': 2, 'TRes': 2, 'eRes': 2, 'GRes': 2, 'DRes': 2,
                                                    'v2Res': 2, 'fRes': 2}],
                    'statescaling': [dict, {}],
                    'residualscaling': [dict, {}],
                    'nffdpoints': [int, 0],
                    ...
                    'derivuininfo': [dict, {'stateName': 'U', 'component': 0, 'type': 'fixedValue',
                                            'patchNames': ['inlet']}],
                    'userdefinedpatchinfo': [dict, {}],
                    'userdefinedvolumeinfo': [dict, {}],
                    ...
                    'preservepatches': [list, []],
                    'singleprocessorfacesets': [list, []],
                    'decomposepardict': [dict, {'method': 'scotch',
    ...
    def __call__(self):
        ...
        if self.getOption('multipointopt') and self.multiPointFCIndex is None:
    ...
    def solveADjoint(self):
        ...
        if not self.skipFlowAndAdjointRuns:
            ...
            if ("FFD" in adjDVTypes) and (self.getOption('updatemesh')):
                ...
                if self.multiPointFCIndex is None or self.multiPointFCIndex == 0:
    ...
    def checkMeshLog(self, logName):
        ...
        if self.comm.rank == 0 and meshOK is False:
    ...
    def _callOpenFoamSolver(self, logFileName):
        ...
        if self.getOption('multipointopt'):
            ...
            if self.multiPointFCIndex is None:
    ...
    def computeAdjointColoring(self):
        ...
        if self.rank == 0:
            print(' Colorings not found, running coloringSolver%s, check the coloringLog file for the progress.' %
        ...
        if self.parallel and mpiSpawnRun:
            ...
            status = subprocess.call('coloringSolver%s' % self.getOption('flowcondition'), stdout=f,
    ...
    def _checkFlowFailure(self, evalFuncs):
        ...
        if not self.getOption('multipointopt'):
            ...
            logFileName = os.path.join(outputDir, 'flowLog_FC%d_%3.3d' % (self.multiPointFCIndex,
                                                                          (self.flowRunsCounter-1)))
    ...
    def _checkAdjointFailure(self):
        ...
        if not self.getOption('multipointopt'):
            ...
            logFileName = os.path.join(outputDir, 'adjointLog_FC%d_%3.3d' % (self.multiPointFCIndex,
                                                                             (self.adjointRunsCounter-1)))
        ...
        ***************************
        ...
        for key in sorted(xDV.keys()):
            ...
            for i in range(lenDVs):
                ...
                # get xSDot
                xSDot = self.DVGeo.totalSensitivityProd(xDvDot, ptSetName=ptSetName,
                                                        comm=self.comm).reshape(xSDot0.shape)
                # get xVDot
    ...
    def _writeDeltaVolPointMat(self, deltaVPointThreshold=1.0E-16):
        ...
        for key in sorted(xDV.keys()):
            if self.comm.rank == 0:
                print ("Writing deltaVolPointMatPlusEps for "+key)
            # get the length of DV for this key
            try:
                lenDVs = len(xDV[key])
            except:  # if xDV[key] is just a number
    ...
    def _readFFDSensitivity(self, obj):
        ...
        objFile = "objFuncsSens_d%sdFFD_%s%3.3d.dat" % (self.possibleObjectives[obj],
                                                        filePrefix, self.adjointRunsCounter-1)
    ...
    def _readUInSensitivity(self, obj):
        ...
        objFile = 'objFuncsSens_d%sdUIn_%s%3.3d.dat' % (self.possibleObjectives[obj],
                                                        filePrefix, self.adjointRunsCounter-1)
        fileName = os.path.join(outputDir, objFile)
    ...
    def _readVisSensitivity(self, obj):
        ...
        objFile = 'objFuncsSens_d%sdVis_%s%3.3d.dat' % (self.possibleObjectives[obj],
                                                        filePrefix, self.adjointRunsCounter-1)
        fileName = os.path.join(outputDir, objFile)
    ...
    def _readObjectiveFunctions(self):
        ...
        if self.getOption('avgobjfuncs'):
            # note we need to read self.flowRunsCounter-1
            costFuncsFileName = os.path.join(outputDir,
                                             "objFuncsMean_%s%3.3d.dat" % (filePrefix,
                                                                           self.flowRunsCounter-1))
        else:
            # note we need to read self.flowRunsCounter-1
            costFuncsFileName = os.path.join(outputDir,
                                             "objFuncs_%s%3.3d.dat" % (filePrefix,
                                                                       self.flowRunsCounter-1))
    ...
    def _cleanSolution(self):
        ...
        if self.parallel:
            # loop over the number of processors, deleting each solution directory along the
            # way
            for i in range(self.comm.size):
    ...
    def _checkRestartLogs(self, logOpt):
        ...
        if logOpt == 1:
            ...
            if not self.getOption('multipointopt'):
                ...
                fileName = os.path.join(outputDir, 'objFuncs_FC%d_%3.3d.dat' % (self.multiPointFCIndex,
                                                                                self.flowRunsCounter))
            ...
            if not self.getOption('multipointopt'):
                ...
                fileName = os.path.join(outputDir, 'designVariables_FC%d_%3.3d.dat' % (self.multiPointFCIndex,
                                                                                       self.adjointRunsCounter))
            ...
            for file1 in allFiles:
                if 'objFuncsSens' in file1:
                    ...
                    if not self.getOption('multipointopt'):
                        ...
                        fileName = os.path.join(outputDir, sensFileName+'_FC%d_%3.3d'
                                                % (self.multiPointFCIndex, self.adjointRunsCounter) + sensFileExt)
    ...
    def _copyLogs(self, logOpt):
        ...
        objective functions and their sensitivities from the outputdirectory. If restartopt=true, The evalFunctions
        ...
        if logOpt == 1:
            ...
            if not self.getOption('multipointopt'):
                ...
                fileCopy = os.path.join(outputDir, 'objFuncs_FC%d_%3.3d.dat' % (self.multiPointFCIndex,
                                                                                self.flowRunsCounter))
            ...
            if not self.getOption('multipointopt'):
                ...
                fileCopy = os.path.join(outputDir, 'designVariables_FC%d_%3.3d.dat' % (self.multiPointFCIndex,
                                                                                       self.adjointRunsCounter))
            ...
            for file1 in allFiles:
                if 'objFuncsSens' in file1:
                    ...
                    if not self.getOption('multipointopt'):
                        ...
                        fileCopy = os.path.join(outputDir,
                                                sensFileName+'_FC%d_%3.3d' % (self.multiPointFCIndex,
                                                                              self.adjointRunsCounter)+sensFileExt)
    ...
    def writeSurfaceSensitivityMap(self, evalFuncs=None, groupName='all', fileType='openfoam'):
        ...
        # generate the folders to save the sensitivity files for openfoam output format
        ...
        for func in evalFuncs:
            ...
            if fileType == 'tecplot':
                ...
                for i in range(len(p0)):
                    ...
                    for j in range(len(points)):
                        fout.write('%f %f %f %f %f %f %f\n' % (points[j][0], points[j][1], points[j][2],
                                                               sens[i][0], sens[i][1], sens[i][2], val))
                for i in range(len(p0)):
                    fout.write('%d %d %d\n' % (3*i+1, 3*i+2, 3*i+3))
                fout.close()
                ...
                # ############### write neighbour
                # note we don't actually need neighbour information for the surface mesh,
                # so we simply assign zeros here
    ...
    def writeSurfaceSensitivityMapFFD(self, evalFuncs, groupName=None):
        ...
        # generate the folder to save the sensitivity files
        if self.comm.size == 1:
            meshDir = '99999/polyMesh/'
            sensDir = '99999/'
            if not os.path.exists(meshDir):
        ...
        for func in evalFuncs:
            ...
            if self.getOption('computeffdderivs'):
                ...
                dIdXs = self.DVGeo.totalSensitivityProd(dIdXFFD, ptSetName=ptSetName,
                                                        comm=self.comm).reshape(dIdXs0.shape)
            else:
    ...
    def _writeTurbulencePropertiesFile(self):
        ...
        if self.comm.rank == 0:
            ...
            if self.getOption('flowcondition') == 'Compressible':
                if 'Prt' not in self.getOption('thermoproperties').keys():
    ...
    def _writeAdjointDictFile(self):
        ...
        if self.comm.rank == 0:
            ...
            f.write('    derivUInInfo           {stateName %s; component %d; type %s;' % (derivUInInfo['stateName'],
                                                                                          derivUInInfo['component'],
                                                                                          derivUInInfo['type']))
            f.write('patchNames (')
